OC.L10N.register(
    "assistant",
    {
    "Summarize using AI" : "透過 AI 生成摘要",
    "Transcribe audio using AI" : "使用 AI 轉錄音訊",
    "Generate subtitles using AI" : "使用 AI 產生字幕",
    "Text-To-Speech using AI" : "使用 AI 進行文字轉語音",
    "Unknown user" : "用戶不詳",
    "Skills folder not found" : "找不到技能資料夾",
    "Failed to list skills" : "無法列出技能",
    "Skill name is too long (max %d characters)" : "技能名稱過長（最多 %d 個字元）",
    "Skill not found" : "找不到技能",
    "Failed to load the skill" : "無法載入技能",
    "Description is too long (max %d characters)" : "描述過長（最多 %d 個字元）",
    "Content is too long (max %d characters)" : "內容過長（最多 %d 個字元）",
    "Failed to store the skill" : "無法儲存技能",
    "Failed to notify when ready; unknown user" : "就緒時通知失敗；用戶不詳",
    "Failed to cancel notification; unknown user" : "取消通知失敗：用戶不詳",
    "Assistant task submitted" : "已提交助理任務",
    "Transcription task submitted" : "已提交轉錄任務",
    "Subtitles task submitted" : "已提交字幕任務",
    "Summarization task submitted" : "已提交摘要任務",
    "Text-to-speech task submitted" : "已提交文字轉語音任務",
    "Send a message to a Talk conversation" : "傳送訊息到 Talk 對話",
    "Create a conversation" : "創建對話",
    "Send an email" : "傳送電子郵件",
    "Schedule a calendar event" : "安排日曆活動",
    "Add a calendar task" : "添加日曆任務",
    "Create a Deck card" : "創建 Deck 卡片",
    "Upload file" : "上傳檔案",
    "Create folder" : "創建文件夾",
    "Move file" : "移動檔案",
    "Copy file" : "複製檔案",
    "Delete file" : "刪除檔案",
    "Create bookmark" : "已建立書籤",
    "Update bookmark" : "更新書籤",
    "Delete bookmark" : "刪除書籤",
    "Create bookmark folder" : "建立書籤資料夾",
    "Create team" : "創建團隊",
    "Add member to team" : "新增成員至團隊",
    "Remove member from team" : "從團隊移除成員",
    "Update team" : "更新團隊",
    "Delete team" : "刪除團隊",
    "Share with team" : "與團隊分享",
    "Create form" : "創建表格",
    "Add question to form" : "新增問題至表單",
    "Delete form" : "刪除表格",
    "Update form settings" : "更新表單設定",
    "Share with user" : "與用戶分享",
    "Share with group" : "與群組分享",
    "Update share permissions" : "更新分享權限",
    "Delete share" : "刪除分享",
    "Failed to create a chat session" : "無法創建聊天時段",
    "Unauthorized" : "未獲授權",
    "Failed to update the chat session" : "無法更新聊天時段",
    "Session not found" : "找不到時段",
    "Could not find session" : "找不到工作階段",
    "User is not logged in" : "用戶未登入",
    "Failed to delete the chat session" : "無法刪除聊天時段",
    "User not logged in" : "用戶未登入",
    "Failed to get chat sessions" : "無法取得聊天時段",
    "Failed to add a chat message" : "無法添加聊天訊息",
    "Failed to get chat messages" : "無法取得聊天訊息",
    "Failed to get chat message" : "無法取得聊天訊息",
    "Failed to delete a chat message" : "無法刪除聊天訊息",
    "Failed to schedule message generation" : "安排訊息產生失敗",
    "Failed to delete the last message" : "無法刪除最後一則訊息",
    "Failed to add a chat message into DB" : "無法將聊天訊息新增至數據庫中",
    "Failed to generate a title for the chat session" : "無法產生聊天時段的標題",
    "User not found" : "找不到用戶",
    "Nextcloud Assistant" : "Nextcloud 小幫手",
    "Assistant task" : "助手任務",
    "AI text generation" : "AI 文字產生",
    "AI image generation" : "人工智能圖像生成",
    "AI audio transcription" : "人工智能音訊轉錄",
    "AI subtitles generation" : "AI 字幕產生",
    "AI context writer" : "人工智能上下文撰寫者",
    "Writing style: %1$s; Source material: %2$s" : "寫作風格：%1$s；來源資料：%2$s",
    "Context Chat" : "上下文聊天",
    "Task for \"%1$s\" has finished" : "「%1$s」任務已結束",
    "\"%1$s\" task for \"%2$s\" has finished" : "「%2$s」的「%1$s」任務已結束",
    "Input: %1$s" : "輸入：%1$s",
    "Result: %1$s" : "結果：%1$s",
    "View results" : "檢視結果",
    "Task for \"%1$s\" has failed" : "「%1$s」任務失敗",
    "\"%1$s\" task for \"%2$s\" has failed" : "「%2$s」的「%1$s」任務失敗",
    "View task" : "檢視任務",
    "File action has finished" : "檔案操作已結束",
    "{sourceFile} has been summarized in {targetFile}" : "{sourceFile} 已總結為 {targetFile}",
    "{sourceFile} has been transcribed in {targetFile}" : "{sourceFile} 已轉錄為 {targetFile}",
    "{sourceFile} has been subtitled in {targetFile}" : "已為 {sourceFile} 加上字幕，並儲存至 {targetFile}",
    "{sourceFile} has been converted to audio in {targetFile}" : "{sourceFile} 已轉換為音頻格式，檔案為 {targetFile}",
    "{sourceFile} has been processed, {targetFile} was created" : "已處理 {sourceFile} 完成。已建立 {targetFile}",
    "File action has failed" : "檔案操作失敗",
    "Summarization of {sourceFile} has failed" : "{sourceFile} 總結失敗",
    "Transcription of {sourceFile} has failed" : "{sourceFile} 轉錄失敗",
    "Subtitling of {sourceFile} has failed" : "為 {sourceFile} 加上字幕失敗",
    "The text-to-speech process for {sourceFile} has failed" : "{sourceFile} 的文字轉語音流程失敗",
    "The processing of {sourceFile} has failed" : "處理 {sourceFile} 失敗",
    "New image file has been generated" : "已產生新圖像檔案",
    "{targetFile} has been generated in {targetDirectory}" : "{targetFile} 已於 {targetDirectory} 產生",
    "Image file generation has failed" : "圖像檔案產生失敗",
    "Generation of a new image file in {targetDirectory} has failed" : "在 {targetDirectory} 中產生新圖像檔失敗",
    "Scheduled task is pending review" : "排程工作正等待審核",
    "\"%s\" awaits your review before continuing." : "「%s」正在等待你的審核後才會繼續。",
    "View scheduled task" : "檢視排程工作",
    "Scheduled task succeeded" : "排程工作已成功完成",
    "\"%s\" was run successfully." : "「%s」已成功執行。",
    "View result" : "檢視結果",
    "Scheduled task failed" : "排程工作失敗",
    "\"%s\" failed to run." : "「%s」執行失敗。",
    "AI sticker generation" : "人工智慧貼圖產生",
    "An error occurred while scheduling this assignment run. Reach out to your system administrator if this issue persists." : "排程此指派執行時發生錯誤。如問題持續，請聯絡你的系統管理員。",
    "Nextcloud Deck" : "Nextcloud Deck",
    "Nextcloud Teams" : "Nextcloud Teams",
    "Nextcloud Tasks" : "Nextcloud 任務",
    "Complete calendar task" : "完成日曆任務",
    "Nextcloud Talk" : "Nextcloud Talk",
    "Nextcloud Files" : "Nextcloud 檔案",
    "OpenProject" : "OpenProject",
    "Delete calendar task" : "刪除日曆任務",
    "DuckDuckGo web search" : "DuckDuckGo 網路搜尋",
    "Nextcloud user profile" : "Nextcloud 使用者個人資料",
    "Nextcloud Calendar" : "Nextcloud 日曆",
    "Nextcloud Contacts" : "Nextcloud 通訊錄",
    "Nextcloud User Search" : "Nextcloud 使用者搜尋",
    "Assistant document generation" : "小幫手文件產生",
    "Assistant image generation" : "小幫手圖像產生",
    "OpenStreetMap" : "OpenStreetMap",
    "Norwegian Meteorological Institute weather forecast" : "挪威氣象局天氣預報",
    "Get file content by link" : "透過連結取得檔案內容",
    "Get form details" : "取得表單詳情",
    "Get form responses" : "取得表單回應",
    "HERE Public transport API" : "HERE 公共交通 API",
    "Get share info" : "取得分享資訊",
    "List bookmark folders" : "列出書籤資料夾",
    "List bookmark tags" : "列出書籤標籤",
    "List bookmarks" : "列出書籤",
    "List forms" : "列出表單",
    "List shares" : "列出分享",
    "List calendar tasks" : "列出日曆任務",
    "List user groups" : "列出用戶群組",
    "Search bookmarks" : "搜尋書籤",
    "Nextcloud Mail" : "Nextcloud 郵件",
    "Assistant audio transcription" : "小幫手音訊轉錄",
    "Update calendar task" : "更新日曆任務",
    "YouTube video search" : "YouTube 影片搜尋",
    "Other" : "其它",
    "Chat with AI" : "與 AI 聊天",
    "Scheduled tasks" : "排程工作",
    "Translate" : "翻譯",
    "Generate file" : "産生檔案",
    "Work with images" : "處理圖像",
    "Work with audio" : "處理音頻",
    "Work with text" : "處理文本",
    "Chat with an AI model." : "與 AI 模型聊天",
    "Generate text" : "生成文字",
    "Send a request to the Assistant, for example: write a first draft of a presentation, give me suggestions for a presentation, write a draft reply to my colleague." : "傳送一個要求給 Assistant，例如：書寫一份演示文稿的初稿，給我演示文稿的建議，書寫一份回覆給我的同事。",
    "The new message is too long" : "新訊息太長了",
    "Message content is empty" : "留言內容是空的",
    "Failed to encode attachments" : "無法編碼附件",
    "Message not found" : "找不到訊息",
    "No user message found in this session" : "此工作階段中找不到使用者訊息",
    "No human message found in this session" : "此工作階段中找不到真人訊息",
    "Assistant" : "助手",
    "Assistant fallback" : "小幫手汰退",
    "Memories" : "回憶",
    "The memories to be injected into the chat session." : "要注入到聊天工作階段的記憶。",
    "Voice chat" : "語音聊天",
    "Voice chat with the assistant" : "與小幫手使用語音聊天",
    "System prompt" : "系統提示詞",
    "Define rules and assumptions that the assistant should follow during the conversation." : "定義小幫手在對話期間應遵守的規則與假設。",
    "Chat voice message" : "聊天語音訊息",
    "Describe a task that you want the assistant to do or ask a question" : "描述您希望助理執行的任務或提出問題",
    "Chat history" : "聊天紀錄",
    "The history of chat messages before the current message, starting with a message by the user" : "目前訊息開始之前的聊天紀錄，從用戶的訊息開始",
    "Input transcript" : "輸入轉錄",
    "Transcription of the audio input" : "語音輸入轉錄",
    "Response voice message" : "回應語音訊息",
    "The generated voice response as part of the conversation" : "産生的語音回應作為對話的一部分",
    "Output transcript" : "輸出轉錄",
    "Transcription of the audio output" : "音訊輸出轉錄",
    "Translation model" : "翻譯模型",
    "The model used to translate" : "用於翻譯的模型",
    "Audio transcription" : "音訊轉錄",
    "The transcribed audio input" : "轉錄後的音訊輸入",
    "Text output" : "文字輸出",
    "The text translation" : "文字翻譯",
    "This was generated using Artificial Intelligence." : "此內容由人工智慧產生。",
    "Translate image" : "翻譯圖像",
    "Translate the text content of an image" : "翻譯圖像文字內容",
    "Input files" : "輸入檔案",
    "The files to extract text from" : "用來提取文字的檔案",
    "Origin language" : "原始語言",
    "The language of the origin text" : "原始文字語言",
    "Target language" : "目標語言",
    "The desired language to translate the origin text in" : "要翻譯原始文字成什麼語言",
    "Output texts" : "輸出文字",
    "The texts that were extracted from the files" : "從檔案擷取的文字",
    "cartoon, neutral background, sticker of %1$s" : "卡通、中性背景、%1$s 的貼圖",
    "Generate sticker" : "産生貼圖",
    "Generate sticker from text" : "從文字産生貼圖",
    "Prompt" : "提示詞",
    "Describe the sticker you would like to create" : "描述您想要產生的貼圖",
    "Output stickers" : "輸出貼圖",
    "The generated sticker" : "産生的貼圖",
    "Process and transform text" : "處理與轉換文字",
    "This app brings a user interface to use the Nextcloud text processing feature.\n\nIt allows users to launch AI tasks, be notified when they finish and see the results.\nThe assistant also appears in others apps like Text to easily process parts of a document.\n\nMore details on the assistant OCS API and frontend integration possibilities in the\n[developer doc](https://github.com/nextcloud/assistant/raw/main/docs/developer)\n\n### How to use it\n\nA new right header menu entry appears. Once clicked, the assistant is displayed and you can select and task type and\nset the input you want to process.\n\nThe task might run immediately or be scheduled depending on the time estimation given by the AI provider.\nOnce a task is scheduled, it will run as a background job. When it is finished, you will receive a notification\nfrom which the results can be displayed.\n\nOther apps can integrate with the assistant. For example, Text will display an inline button besides every paragraph\nto directly select a task type to process this paragraph. Selecting a task this way will open the assistant with the task\nbeing pre-selected and the input text set.\n\nMore details and screenshots in the [user doc](https://github.com/nextcloud/assistant/raw/main/docs/user).\n\n## Features\n\nIn the assistant, the list of available tasks depends on the available providers installed via other apps.\nThis means you have complete freedom over which service/software will actually run your AI tasks.\n\n### Text processing providers\n\nSo far, the [Local Large language model](https://github.com/nextcloud/llm2#readme)\nand the [OpenAi/LocalAI integration](https://apps.nextcloud.com/apps/integration_openai) apps\ninclude text processing providers to:\n* Summarize\n* Extract topics\n* Generate a headline\n* Get an answer from a free prompt\n* Reformulate (OpenAi/LocalAi only)\n* Context writer: Generate text with a specified style. The style can be described or provided via an example text.\n\n### Text to image (Image generation)\n\nKnown providers:\n* [OpenAi/LocalAI integration](https://apps.nextcloud.com/apps/integration_openai)\n* [Text2Image Stable Diffusion](https://apps.nextcloud.com/apps/text2image_stablediffusion)\n\n### Speech to text (Audio transcription)\n\nKnown providers:\n* [OpenAi/LocalAI integration](https://apps.nextcloud.com/apps/integration_openai)\n* [Local Whisper Speech-To-Text](https://apps.nextcloud.com/apps/stt_whisper)\n\nMore details on how to set this up in the [admin docs](https://docs.nextcloud.com/server/latest/admin_manual/ai/index.html)" : "此應用程式帶來了一個使用 Nextcloud 文字處理功能的使用者介面。\n\n其讓使用者可以啟動文字處理任務，在任務完成時收到通知並檢視結果。\n小幫手也會出現在「文字」等其他應用程式中，可以輕鬆處理文件的某些部分。\n\n更多關於小幫手 OCS API 與前端整合可能性的詳細資訊請見\n[開發者文件](https://github.com/nextcloud/assistant/raw/main/docs/developer)\n\n### 如何使用它\n\n出現新的右標題選單條目。點擊後，將顯示小幫手，您可以選擇任務類型與\n設定要處理的輸入文字。\n\n規劃好任務後，其將作為背景作業執行。完成後，您將會收到其中顯示結果的通知。\n\n其他應用程式可以與小幫手應用程式整合。例如，文字將在每個段落旁邊顯示一個行內按鈕，直接選擇一個任務類型來處理該段落。以這種方式選擇任務將會開啟包含該任務的助手，且預先選擇並設定輸入文字。\n\n在[使用者文件](https://github.com/nextcloud/assistant/raw/main/docs/user)中有更多詳細資訊與螢幕截圖。\n\n## 功能\n\n在小幫手中，可用任務清單取決於透過其他應用程式安裝的可用提供者。\n這代表了您可以完全自由地決定哪些服務/軟體實際執行您的文字處理任務。\n\n### 文字處理提供者\n\n到目前為止，[本機大型語言模型](https://github.com/nextcloud/llm2#readme)以及 [OpenAi/LocalAI 整合](https://apps.nextcloud.com/apps/integration_openai) 應用程式包含了文字處理提供者：\n* 總結\n* 擷取主題\n* 產生標題\n* 從免費提示中取得答案\n* 重新表述（僅限 OpenAI/LocalAI）\n* 情境作家：產生指定樣式的文字。可以透過範例文字來描述或提供樣式。\n\n已知提供者：\n* [OpenAI/LocalAI 整合](https://apps.nextcloud.com/apps/integration_openai)\n* [文字轉圖像 Stable Diffusion](https://apps.nextcloud.com/apps/text2image_stablediffusion)\n\n### 語音轉文字（音訊轉錄）\n\nKnown providers:\n* [OpenAI/LocalAI 整合](https://apps.nextcloud.com/apps/integration_openai)\n* [本機 Whisper 語音轉文字](https://apps.nextcloud.com/apps/stt_whisper)\n\n更多關於如何設定的資訊，請見[管理文件](https://docs.nextcloud.com/server/latest/admin_manual/ai/index.html)",
    "Select global skills folder" : "選擇全域技能資料夾",
    "Select" : "選擇",
    "No folder selected" : "尚未選擇資料夾",
    "Global skills folder updated" : "已更新全域技能資料夾",
    "Failed to set global skills folder" : "無法設定全域技能資料夾",
    "Assistant admin options saved" : "已儲存 Assistant 管理員選項",
    "Failed to save assistant admin options" : "儲存 Assistant 管理選項失敗",
    "Find more details on how to set up Assistant and recommended backends in the Administration documentation." : "在管理文件中尋找更多關於如何設定小幫手與建議後端的資訊。",
    "Administration documentation" : "管理使用手冊",
    "The sticker generation feature won`t work without being able to generate images. Please install and enable a \"Generate image\" provider or disable the \"Generate sticker\" task type." : "貼紙產生功能需要可用的圖片產生服務才能運作。請安裝並啟用「圖片產生」供應者，或停用「產生貼紙」工作類型。",
    "Select which features you want to enable" : "選取要啟用的功能",
    "Nextcloud Assistant in header" : "導覽列中的 Nextcloud 小幫手",
    "AI text generation in the smart picker" : "智慧型挑選程式中的人工智慧文字產生",
    "Text-to-image in the smart picker" : "智慧型挑選程式中的文字轉圖像",
    "Text-to-sticker in the smart picker" : "智慧型挑選程式中的文字轉貼圖",
    "Speech-to-text in the smart picker" : "智慧型挑選程式中的語音轉文字",
    "To be able to use the Assistant, please install at least one AI task processing provider." : "要使用小幫手，請安裝至少一個人工智慧任務處理提供者。",
    "To enable text generation in the smart picker, please install an AI task processing provider for the \"Free text to text prompt\" task type:" : "要在智能挑選器中啟用文字產生功能，請為「自由文字轉文字提示」工作類型安裝一個 AI 工作處理供應者：",
    "To enable the sticker generation picker or the image generation picker, please install and enable a \"Generate image\" provider:" : "要啟用貼紙生成選擇器或圖像生成選擇器，請安裝並啟用 '生成圖像' 提供者：",
    "To enable speech-to-text in the smart picker, please install and enable a \"Transcribe audio\" provider:" : "要在智能選擇器中啟用語音轉文本，請安裝並啟用 '轉錄音頻' 提供者：",
    "Data folder" : "資料儲存位置",
    "Name of the folder the assistant creates in the files of each user to store generated content. Users can override this in their personal settings. Changing it does not rename folders that already exist." : "小幫手會在每位使用者的檔案中建立一個資料夾，用以儲存產生的內容，該資料夾的名稱即為此。使用者可在個人設定中覆寫此名稱。變更此名稱並不會重新命名現有的資料夾。",
    "Default data folder name" : "默認數據資料夾名稱",
    "Chat User Instructions for Chat Completions" : "聊天用戶完成聊天的說明",
    "It is passed on to the LLM for it to better understand the context." : "傳達給大型語言模型，以便它更好地理解上下文。",
    "\"{user}\" is a placeholder for the user's display name." : "“{user}”是用戶顯示名稱的佔位符。",
    "Chat User Instructions for Title Generation" : "聊天用戶産生標題的說明",
    "It is passed on to the LLMs to better generate a chat title" : "此任務交由大型語言模型處理，以更有效地生成對話標題。",
    "\"{user}\" is a placeholder for the user's display name here as well." : "\"{user}\" 也是此處用戶顯示名稱的佔位符。",
    "Last N messages to consider for chat completions" : "考慮用於完成聊天的最後 N 條消息",
    "This includes the user instructions and the LLM's messages" : "這包括用戶指令和大型語言模型的訊息",
    "Number of messages to consider for chat completions (excluding the user instructions, which is always considered)" : "考慮用於聊天完成的訊息數量（不包括一律考慮的用戶指令）",
    "Global Agent Skills" : "全域代理技能",
    "Select a folder containing skills (sub-folders, each holding a SKILL.md file). These will be available to all users in addition to their personal skills in \"Chat with AI\". The folder is resolved from the filesystem of the admin who set it." : "選擇一個包含技能的資料夾（每個子資料夾各自包含一個 SKILL.md 檔案）。除「與 AI 聊天」中的個人技能外，所有用戶亦可使用這些技能。系統會從設定此資料夾的管理員檔案系統中解析其位置。",
    "Clear" : "清除",
    "Selected folder" : "所選資料夾",
    "Set by" : "設定者",
    "No global skills folder configured" : "尚未設定全域技能資料夾",
    "The task ran successfully but the result is identical to the input." : "任務成功運行，但結果與輸入相同。",
    "This output was generated by AI. Make sure to double-check and adjust." : "此輸出是由人工智慧產生的。請務必仔細檢查與調整。",
    "Nextcloud Assistant header menu entry" : "Nextcloud 小助手標頭選項單項目記錄",
    "Hide advanced options" : "隱藏進階選項",
    "Show advanced options" : "顯示進階選項",
    "Try again" : "重试",
    "Send request" : "傳送請求",
    "Launch this task again" : "再次啟動此任務",
    "Launch a task" : "執行任務",
    "Failed to parse some files" : "部份檔案解析失敗",
    "Generate a first draft for a blog post about privacy" : "為隱私的部落格文章產生初稿",
    "What is the venue for the team retreat this quarter?" : "本季團隊靜修地點在哪裡？",
    "Type or paste the text to summarize" : "輸入或貼上文字進行總結",
    "Type or paste the text to generate a headline for" : "輸入或貼上文字以產生標題",
    "Type or paste the text to extract the topics from" : "輸入或貼上文字以從中提取主題",
    "landscape trees forest peaceful" : "風景 樹木 森林 寧靜",
    "a number" : "一個號碼號碼",
    "Shakespeare or an example of the style" : "莎士比亞或這種風格的例子",
    "A description of what you need or some original content" : "您需要什麼或一些原創內容的描述",
    "New task" : "新任務",
    "Provider name" : "提供者名字",
    "Provider: {name}" : "提供者：{name}",
    "Getting results…" : "正在取得結果…",
    "Get notified when the task finishes" : "任務完成通知",
    "Cancel task" : "取消任務",
    "Close" : "關閉",
    "Close Nextcloud Assistant" : "關閉 Nextcloud 小幫手",
    "Less" : "更少",
    "More" : "更多",
    "If you are not satisfied with the actions the Assistant wants to run, you can adjust your request by sending a new message instead of clicking the \"Cancel\" button." : "如果您不滿意小幫手要執行的動作，您可以傳送新訊息來調整您的要求，而非按「取消」按鈕。",
    "The Assistant wants to perform sensitive actions on your behalf." : "小幫手想要代表您執行敏感動作。",
    "Cancel" : "取消",
    "Confirm those actions" : "確認這些動作",
    "You" : "您",
    "Message copied to clipboard" : "訊息已複製到剪貼板",
    "MCP server: {tool_id}" : "MCP 伺服器：{tool_id}",
    "Reasoning content" : "推理內容",
    "Information sources & actions" : "資訊來源及動作",
    "Reasoning…" : "正在推理…",
    "Ask assistant, what's the weather today" : "詢問小幫手：今天天氣如何",
    "What's the weather today?" : "今天天氣如何？",
    "Ask assistant, to create a share link for a file" : "請小幫手為檔案建立分享連結",
    "Can you create a share link for me?" : "你可以為我建立分享連結嗎？",
    "Ask assistant, to create a scheduled task to send me the weather every morning" : "請助理建立排程工作，每朝早傳送天氣資訊給我",
    "Create a scheduled task to send me the weather every morning " : "建立排程工作，每朝早傳送天氣資訊給我",
    "Ask assistant, which actions it can do for you" : "詢問小幫手：它能為您執行哪些動作",
    "Which actions can you do for me?" : "你可以幫我執行哪些動作？",
    "Ask assistant for route from Munich to Berlin using public transport" : "請助理提供由慕尼黑乘搭公共交通工具前往柏林的路線",
    "Can you give me a route from Munich to Berlin using public transport?" : "可以提供由慕尼黑乘搭公共交通工具前往柏林的路線嗎？",
    "Ask assistant to transcribe a media file for you" : "請助理為您轉錄媒體檔案",
    "Transcribe a media file for me" : "為我轉錄媒體檔案",
    "Ask assistant to schedule an event for a Design meeting whenever you're free tomorrow" : "請助理安排明天我有空時舉行設計會議",
    "Schedule an event for a Design meeting whenever I'm free tomorrow" : "安排明天我有空時舉行設計會議",
    "Ask assistant to create a deck card in my Project board in the todo stack for creating the presentation" : "請助理在我的「Project」看板「todo」清單中建立一張 Deck 工作卡，用於製作簡報",
    "Create a deck card in my Project board in the todo stack for creating the presentation" : "在我的「Project」看板「todo」清單中建立一張 Deck 工作卡，用於製作簡報",
    "Ask assistant to generate a slide deck about the features of Nextcloud" : "請助理產生一份介紹 Nextcloud 功能的投影片簡報",
    "Generate a slide deck about the features of Nextcloud" : "產生一份介紹 Nextcloud 功能的投影片簡報",
    "Ask assistant to generate an image of a puppy with a Nextcloud hat" : "請助理產生一張戴着 Nextcloud 帽子的小狗圖片",
    "Generate an image of a puppy with a Nextcloud hat" : "產生一張戴着 Nextcloud 帽子的小狗圖片",
    "Are you sure you want to delete \"{sessionTitle}\"?" : "您確定您想要刪除「{sessionTitle}」嗎？",
    "Error generating a response" : "産生回應對時發生錯誤",
    "Error getting the generated title for the conversation" : "獲取對話生成標題時發生錯誤",
    "Error checking if the session is thinking" : "檢查工作階段是否正在思考時發生錯誤",
    "Error updating title of conversation" : "更新對話標題時發生錯誤",
    "Untitled conversation" : "未命名對話",
    "Error generating a title for the conversation" : "生成對話標題時出現錯誤",
    "Error deleting conversation" : "刪除對話時發生錯誤",
    "Error fetching assignments" : "擷取指派項目時發生錯誤",
    "Error fetching conversations" : "擷取對話時發生錯誤",
    "Error deleting message" : "刪除訊息時發生錯誤",
    "Error fetching messages" : "擷取訊息時發生錯誤",
    "Error creating a new message" : "創建新訊息時發生錯誤",
    "Invalid response received for a new conversation request" : "收到無效的回應，無法建立新對話請求",
    "Error creating a new conversation" : "創建新對話時發生錯誤",
    "Error regenerating a response" : "重新産生回應時出現錯誤",
    "New conversation" : "新對話",
    "Loading scheduled tasks…" : "正在載入排程工作…",
    "Loading conversations…" : "對話加載中 ...",
    "No scheduled tasks yet" : "尚未有排程工作",
    "No conversations yet" : "尚無對話",
    "Delete" : "刪除",
    "Conversation title" : "對話標題",
    "Prompt: {prompt}" : "提示：{prompt}",
    "Recurrence: {recurrence}" : "重複週期：{recurrence}",
    "Remember this" : "記住此",
    "Edit title" : "編輯標題",
    "Generate title" : "産生標題",
    "Creating a new conversation" : "創建新對話",
    "No results yet" : "尚未有結果",
    "This task has not run yet. Results will appear here after the next scheduled run." : "此工作尚未執行。下一次排程執行後，結果將顯示於此。",
    "Scheduled tasks run automatically on a recurring schedule. Ask chat to create one for you." : "排程工作會按重複週期自動執行。請在聊天中要求建立一項排程工作。",
    "Create in chat" : "在聊天中建立",
    "Hello there! What can I help you with today?" : "你好！今天我能幫你什麼忙呢？",
    "Try sending a message to spark a conversation." : "試著發送一條訊息來引發對話。",
    "Load older messages" : "加載較舊的消息",
    "Retry response generation" : "請重試回應生成",
    "Output shown here is generated by AI. Make sure to always double-check." : "此處顯示的輸出是由人工智慧產生的。請務必仔細檢查。",
    "Messages should not be longer than {maxLength} characters (currently {length})." : "訊息不應長於 {maxLength} 個字元（目前 {length}）。",
    "Conversation deletion" : "刪除對話",
    "Error loading messages" : "加載訊息時發生錯誤",
    "Loading messages…" : "正在加載訊息 ...",
    "Remove this media" : "移除此媒體",
    "The text must be shorter than or equal to {maxLength} characters, currently {length}" : "目前文本長度為 {length}，必須小於或等於 {maxLength} 個字符",
    "Cancel editing" : "取消編輯",
    "Submit" : "遞交",
    "Type a message…" : "請輸入一則訊息 ...",
    "Processing…" : "處理中 ...",
    "Waiting…" : "正在等待……",
    "Attach a file" : "附加檔案",
    "Upload from device" : "從裝置上傳",
    "Select from nextcloud" : "從 Nextcloud 選擇",
    "Choose a file" : "選擇一個檔案",
    "Choose" : "選擇",
    "Task position: {position}" : "任務位置：{position}",
    "Could not upload the recorded file" : "無法上傳錄製檔案",
    "Please try again and inform the server administrators if this issue persists." : "請再試一次，如果問題仍然存在，請通知伺服器管理員。",
    "Could not upload the file" : "無法上傳檔案",
    "No file selected" : "未選擇任何檔案",
    "This chat response is taking longer to start generating than expected. Please contact your administrator to ensure that Assistant is correctly configured." : "此聊天回應開始產生的時間比預期長。請聯絡您的管理員，確保小幫手已正確設定。",
    "Copy message" : "複製訊息",
    "Regenerate message" : "重新生成訊息",
    "Delete message" : "刪除訊息",
    "Select Files/Folders" : "選擇檔案／資料夾",
    "Select Providers" : "選擇提供者",
    "Choose Files/Folders" : "選擇檔案／資料夾",
    "Clear Selection" : "清除選擇",
    "Error fetching default provider key" : "擷取默認提供者密鑰時發生錯誤",
    "Error fetching providers" : "擷取提供者時發生錯誤",
    "Context Chat has not finished indexing all your documents yet, it may not be able to answer your questions, yet." : "上下文聊天尚未完成索引所有文檔，因此目前可能無法回答您的問題。",
    "Selective context" : "選擇性上下文",
    "No sources referenced" : "沒有引用來源",
    "Prompt cannot be empty." : "提示詞不能為空。",
    "Image generation task has been scheduled." : "已排定圖像產生任務。",
    "Failed to schedule image generation task." : "安排圖像產生任務失敗。",
    "Generate new image using AI" : "使用 AI 生成新圖片",
    "The result image would be placed in the current folder." : "結果圖像會放在目前的資料夾中。",
    "The resulting images are generated by AI. Make sure to double-check and adjust." : "生成的圖片由 AI 產生，請務必再次檢查並作出調整。",
    "Generate new image" : "產生新圖像",
    "Describe the image you want to generate" : "描述您想要產生的圖像",
    "No providers found" : "找不到提供者",
    "Image generation is not available." : "圖像產生不可用。",
    "Generate image" : "産生圖像",
    "tool" : "工具",
    "integration" : "整合",
    "complete AI documentation" : "完整人工智能說明書",
    "AI provider apps can be found in the {toolLink} and {integrationLink} app settings sections." : "人工智能提供者應用程式可在 {toolLink} 與 {integrationLink} 的應用程式設定區塊中找到。",
    "You can also check the {aiAdminDocLink}" : "您也可以看看 {aiAdminDocLink}",
    "No provider found" : "找不到提供者",
    "AI Providers need to be installed to use the Assistant" : "必須安裝人工智慧提供者以使用小幫手",
    "Assistant options saved" : "已儲存 Assistant 選項",
    "Failed to save assistant options" : "儲存 Assistant 選項失敗",
    "Auto-play audio chat responses" : "自動播放音訊聊天回應",
    "AI text generation in smart picker" : "智慧型挑選程式中的人工智慧文字產生",
    "AI image generation in smart picker" : "智慧型挑選程式中的人工智慧圖像產生",
    "AI sticker generation in smart picker" : "智慧型挑選程式中的人工智慧貼圖產生",
    "AI transcription in smart picker" : "智慧型挑選程式中的人工智慧轉錄",
    "No suitable providers are available. They must first be enabled by your administrator." : "沒有可用的合適提供者。它們必須先由您的管理員啟用。",
    "Configured backends" : "已配置的後端",
    "The following services are used as backends for Nextcloud Assistant:" : "以下服務用作 Nextcloud 小幫手的後端：",
    "Where the assistant stores content it generates for you. Leave empty to use the name your administrator has set. Changing it does not move files that are already there." : "小幫手將為您產生的內容儲存於此處。若留空，則會使用管理員所設定的名稱。變更此設定並不會移動現有的檔案。",
    "Folder name" : "資料夾名稱",
    "Remembered conversations" : "已記住的對話",
    "The following conversations are remembered by the Assistant Chat and will be taken into account for every new conversation:" : "以下對話已被 Assistant Chat 記住，並會在每次新的對話中一併考慮：",
    "This may take a few seconds…" : "此操作可能需要花幾秒鐘 ...",
    "This may take a few minutes…" : "這可能需要幾分鐘…",
    "This task is running in the background." : "此工作正在背景執行。",
    "You can safely close the assistant or browse other tasks." : "您可以放心關閉助理或瀏覽其他工作。",
    "This task is taking longer to start running than expected. Please contact your administrator to ensure that Assistant is correctly configured." : "此任務開始執行的時間比預期長。請聯絡您的管理員，確保小幫手已正確設定。",
    "You have not submitted any \"{taskTypeName}\" task yet" : "您尚未提交任何 \"{taskTypeName}\" 任務",
    "Nothing yet" : "暫無資料",
    "Succeeded" : "成功了",
    "Cancelled" : "已取消",
    "Failed" : "失敗了",
    "Running" : "跑步",
    "Scheduled" : "預定",
    "_{n} image generated_::_{n} images generated_" : ["生成了 {n} 張圖片"],
    "_{n} image scheduled_::_{n} images scheduled_" : ["計劃了 {n} 張圖片"],
    "Task scheduled" : "任務已計劃",
    "Unknown status" : "狀態不詳",
    "Audio input" : "音頻輸入",
    "Audio output" : "音頻輸出",
    "Access to the microphone was denied. Please check you allowed this page to access the microphone." : "存取米高風被拒。請檢查您是否允許此頁面存取米高風。",
    "Microphone either not available or disabled in settings. Check you are accessing this page with HTTPS or adjust your browser settings." : "米高風無法使用或在設定中停用。請檢查您是否使用 HTTPS 存取此頁面，或調整您的瀏覽器設定。",
    "Error while recording audio" : "錄音時出錯",
    "Start recording" : "開始錄音",
    "Dismiss recording" : "撤銷錄音",
    "End recording and send" : "結束錄音並傳送",
    "Choose file" : "選擇檔案",
    "Choose a value" : "選擇一個數值",
    "Output file share link copied to clipboard" : "輸出檔案分享連結已複製到剪貼簿",
    "Click this to open the file" : "點擊此處以打開文件",
    "This output file has been saved in {path}" : "此輸出檔案已保存至 {path}",
    "Could not copy to clipboard" : "無法複製到剪貼簿",
    "Could not upload the files" : "無法上傳檔案",
    "Drop files here to upload them" : "將檔案拖拽到此處即可上傳",
    "Select from Nextcloud" : "從 Nextcloud 選擇",
    "Pick one or multiple files" : "選擇一個或多個檔案",
    "Download this media" : "下載此媒體",
    "Save this media" : "保存此媒體",
    "Share this media" : "分享此媒體",
    "Drop a file here to upload it" : "將檔案拖拽到此處即可上傳",
    "Select from storage" : "從儲存空間選取",
    "Pick a file" : "挑選一個檔案",
    "Clear value" : "清除數值",
    "Type some number" : "輸入某些號碼",
    "The current value is not a number" : "目前數值不是數字",
    "Custom value" : "自訂值",
    "Advanced" : "進階",
    "Choose a text file" : "選擇文字檔案",
    "Warning: The input text exceeds the maximum length of {limit} characters (currently {length})." : "警告：輸入文字超過最多的 {limit} 個字元（目前 {length}）。",
    "Unexpected response from text parser" : "文字解析程式回應異常。",
    "Could not parse file" : "無法分析檔案",
    "Result could not be copied to clipboard" : "結果無法複製到剪貼板。",
    "Double-click to edit" : "雙擊以編輯",
    "Improve with new instructions" : "使用新指示改善",
    "Improve this text" : "改善此文字",
    "Copy output" : "複製輸出",
    "Copy" : "複製",
    "Your task with ID {id} has failed" : "您的任務（ID 為 {id}）失敗",
    "The current Assistant task could not be found" : "找不到目前的小幫手任務",
    "Assistant error" : "Assistant 錯誤",
    "Something went wrong when scheduling the task" : "排程任務時發生錯誤",
    "Submit the current task's result" : "遞交目前任務的結果",
    "Output file for task {taskId} ({taskTypeName})" : "任務 {taskId} 的輸出檔案（{taskTypeName}）",
    "The server failed to process your task with ID {id}" : "伺服器無法處理您 ID 為 {id} 的任務",
    "Please inform the server administrators of this issue." : "請通知伺服器管理員此問題。",
    "No last task type available, falling back to default" : "沒有可用的最後任務類型，回落到默認值。",
    "Please log in to view the task result" : "請登入查看任務結果",
    "This task does not exist or has been cleaned up" : "任務不存在或已被刪除",
    "Assistant failed to schedule your task" : "小幫手無法安排您的任務。",
    "Does not repeat" : "不要重複",
    "Minutely" : "每分鐘",
    "Hourly" : "每小時",
    "Daily" : "每日",
    "Weekly" : "每星期",
    "Monthly" : "每月",
    "Yearly" : "每年",
    "_Every %n minute_::_Every %n minutes_" : ["每 %n 分鐘"],
    "_Every %n hour_::_Every %n hours_" : ["每%n小時"],
    "_Every %n day_::_Every %n days_" : ["每 %n 日"],
    "_Every %n week_::_Every %n weeks_" : ["每 %n 星期"],
    "_Every %n month_::_Every %n months_" : ["每 %n 月"],
    "_Every %n year_::_Every %n years_" : ["每 %n 年"],
    "_on {weekday}_::_on {weekdays}_" : ["於 {weekdays}"],
    "_on day {dayOfMonthList}_::_on days {dayOfMonthList}_" : ["在 {dayOfMonthList} 天"],
    "on the {ordinalNumber} {byDaySet}" : "在 {ordinalNumber} {byDaySet} 上",
    "in {monthNames} on the {dayOfMonthList}" : "在 {monthNames} 的 {dayOfMonthList}",
    "in {monthNames} on the {ordinalNumber} {byDaySet}" : "在 {ordinalNumber} {byDaySet} 於 {monthNames}",
    "until {untilDate}" : "到 {untilDate}",
    "_%n time_::_%n times_" : ["%n 次"],
    "first" : "第一個",
    "second" : "秒",
    "third" : "第三",
    "fourth" : "第四",
    "fifth" : "第五",
    "second to last" : "尾二",
    "last" : "最後一個",
    "Generate image using AI" : "使用 AI 生成圖片",
    "AI Assistant" : "AI 助手",
    "Summarization AI task submitted." : "已提交 AI 摘要任務。",
    "You will be notified when it is ready." : "就緒時將會通知您。",
    "It can also be checked in the Assistant in the \"Work with text -> Summarize\" menu." : "也可以在小幫手的「處理文字」→「總結」選單中確認。",
    "Failed to launch the AI file action" : "啟動人工智慧檔案動作失敗",
    "Text-to-Speech AI task submitted." : "已提交 AI 文字轉語音任務。",
    "It can also be checked in the Assistant in the \"Work with audio -> Generate speech\" menu." : "也可以在小幫手的「處理音訊」→「產生語音」選單中確認。",
    "Transcription AI task submitted." : "已提交 AI 轉錄任務。",
    "It can also be checked in the Assistant in the \"Work with audio -> Transcribe audio\" menu." : "也可以在小幫手的「處理音訊」→「轉錄語音」選單中確認。",
    "AI subtitles task submitted." : "已提交 AI 字幕任務。",
    "It can also be checked in the Assistant in the \"Work with audio -> Generate subtitles\" menu." : "您亦可在助理的「處理音訊 → 產生字幕」選單中查看。",
    "Assistant task submitted successfully" : "助手任務已成功遞交",
    "Transcription task submitted successfully" : "轉錄任務已成功遞交",
    "Summarization task submitted successfully" : "總結任務已成功遞交",
    "Text-to-speech task submitted successfully" : "文字轉語音任務遞交成功",
    "Information sources" : "資訊來源",
    "Summarization AI task submitted successfully." : "AI 摘要任務已成功提交。",
    "Text-to-Speech AI task submitted successfully." : "AI 文字轉語音任務已成功提交。",
    "Transcription AI task submitted successfully." : "AI 轉錄任務已成功提交。"
},
"nplurals=1; plural=0;");
